...
export default function Buscar() {
  ...
  const allData = async () => {
    ...
    const data = await context.getPlaylistsByUser(num, text);
    console.log(num)
    if (data) {
  ...
  <div className="flex justify-center bg-slate-900 ">
    <div className="mt-10 border rounded p-8 max-w-md w-full bg-white shadow-lg shadow-white ">
      <div className="mb-5 flex justify-center items-center  ">
        ...
        <h5 className='text-xl text-center font-bold'> Todas las playlist</h5>
        </form>
      </div>
      <div className="mt-5 flex justify-between items-center">
        <h5>PlayList</h5>
        <button
          className="ml-10 bg-cyan-700 hover:bg-red-700 text-white py-1 w-full rounded-md hover:text-white font-semibold pointer-events-auto"
          onClick={() => {
      ...
      <div className="mt-4">
        <table className="min-w-full rounded border-gray-400 border ">
          <thead>
            <tr>
              <th className="py-2 rounded border-gray-400 border ">Título</th>
              
              <th className="py-2 rounded border-gray-400 border">Descripción</th>
